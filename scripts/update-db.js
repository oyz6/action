...
"US","CA","BM","KY","VG", // ✅ 补上百慕大、开曼群岛、英属维尔京群岛
];
...
if (p.length !== 4 || p.some(x => isNaN(x) || x < 0 || x> 255)) return false;
    ...
    if (a === 100 && b >= 64 && b <= 127) return false; if (a===127) return false; if (a===169 && b===254) return false;
        if (a===172 && b>= 16 && b <= 31) return false; if (a===192 && b===0 && c===2) return false; if (a===192 &&
            b===168) return false; if (a===198 && (b===18 || b===19)) return false; if (a===198 && b===51 && c===100)
            return false; if (a===203 && b===0 && c===113) return false; if (a>= 224) return false;
            return true;
            }
            ...
            let n = ((p[0] << 24) | (p[1] << 16) | (p[2] << 8) | p[3])>>> 0;
                ...
                const out = [];
                for (let i = 0; i < arr.length; i +=size) out.push(arr.slice(i, i + size)); return out; }
                    //=============================================// 解析 delegated 文件
                    //=============================================function parseDelegatedFile(text, targetCountries) {
                    const pool={}; const targetSet=new Set(targetCountries); for (const line of text.split("\n")) {
                    const trimmed=line.trim(); if (!trimmed || trimmed.startsWith("#")) continue; const
                    p=trimmed.split("|"); if (p.length < 7) continue; if (p[1]==="*" ) continue; if (p[2] !=="ipv4" )
                    continue; const status=p[6]?.split("#")[0].trim(); if (status==="summary" ) continue; const
                    cc=p[1]?.toUpperCase(); const startIP=p[3]; const count=parseInt(p[4]) || 0; if (!cc || cc.length
                    !==2) continue; if (!targetSet.has(cc)) continue; if (!isPublicIP(startIP)) continue; if (count <
                    256) continue; if (!pool[cc]) pool[cc]=[]; if (pool[cc].length < 50) { pool[cc].push({ startIP,
                    count }); } } return pool; } function sampleFromBlocks(blocks, n) { const ips=[]; const
                    step=Math.max(1, Math.floor(blocks.length / n)); for (let i=0; i < blocks.length && ips.length < n;
                    i +=step) { const { startIP, count }=blocks[i]; const offset=Math.min(Math.floor(count / 2), 100);
                    const ip=addOffset(startIP, offset); if (ip) ips.push(ip); } return ips; }
                    //=============================================// MaxMind 本地验证
                    //=============================================let lookupDb=null; async function initMaxMind() { if
                    (!lookupDb) { const dbPath=path.join(process.cwd(), 'data' , 'GeoLite2-Country.mmdb' );
                    lookupDb=await maxmind.open(dbPath); } return lookupDb; } async function verifyWithMaxMind(ipList) {
                    const db=await initMaxMind(); const result={}; for (const ip of ipList) { try { const
                    geo=db.get(ip); if (geo?.country?.iso_code) { result[ip]=geo.country.iso_code.toUpperCase(); } }
                    catch (e) { /* ignore */ } } return result; } //=============================================//
                    mra8-api 查询 //=============================================async function queryMRA8(ip) { try {
                    const resp=await fetch(`https://mra8-api.hf.space/${ip}`, { signal: AbortSignal.timeout(5000), });
                    if (!resp.ok) return null; const data=await resp.json(); return data?.country?.code?.toUpperCase()
                    || null; } catch (e) { return null; } } //=============================================//
                    抓取与Bypass动态逻辑 //=============================================let RIR_RAW_TEXT={}; function
                    getAutoBypassIPs(cc) { const ips=[]; for (const [rir, text] of Object.entries(RIR_RAW_TEXT)) { const
                    lines=text.split('\n'); for (const line of lines) { if (line.startsWith('#') || line.trim()==='' )
                    continue; const p=line.split('|'); if (p.length < 7 || p[1] !==cc || p[2] !=="ipv4" ) continue;
                    const status=p[6]?.split('#')[0].trim(); if (status==="summary" ) continue; const startIP=p[3];
                    const count=parseInt(p[4]) || 0; if (!isPublicIP(startIP) || count < 8) continue; const
                    mid=addOffset(startIP, Math.floor(count / 2)); if (mid && !ips.includes(mid)) ips.push(mid); } }
                    return [...new Set(ips)].slice(0, 5); } function getTerritoryFallbackIPs(cc) { const
                    prefixes=TERRITORIES_FALLBACK[cc] || []; const ips=[]; for (const pfx of prefixes) { const [base,
                    bits]=pfx.split('/'); const size=Math.pow(2, 32 - parseInt(bits)); const ip=addOffset(base,
                    Math.floor(size / 2)); if (ip) ips.push(ip); } return ips; } function getXKCandidatesFromRIPE() {
                    const ripeText=RIR_RAW_TEXT["RIPE"]; if (!ripeText) return []; const ips=[]; const
                    lines=ripeText.split('\n'); for (const line of lines) { if (line.startsWith('#') || line.trim()===''
                    ) continue; const p=line.split('|'); if (p.length < 7 || p[1] !=="XK" || p[2] !=="ipv4" ) continue;
                    const status=p[6]?.split('#')[0].trim(); if (status==="summary" ) continue; const startIP=p[3];
                    const count=parseInt(p[4]) || 0; if (!isPublicIP(startIP) || count < 256) continue; const
                    first=addOffset(startIP, 1); const mid=addOffset(startIP, Math.floor(count / 2)); const
                    last=addOffset(startIP, count - 2); for (const ip of [first, mid, last]) { if (ip &&
                    !ips.includes(ip)) ips.push(ip); } } return ips.slice(0, 30); }
                    //=============================================// 抓取 RIR（使用稳定镜像）
                    //=============================================async function fetchFromRIR(url, targetCountries,
                    name) { console.log(`[${name}] 抓取中...`); try { const resp=await fetch(url, { signal:
                    AbortSignal.timeout(60000), headers: { "User-Agent" : "GH-IPCollector/5.0" }, }); if (!resp.ok) {
                    console.log(`[${name}] HTTP ${resp.status}`); return { candidates: {}, raw: null }; } const
                    text=await resp.text(); console.log(`[${name}] ${(text.length / 1024).toFixed(0)} KB`);
                    RIR_RAW_TEXT[name]=text; const pool=parseDelegatedFile(text, targetCountries);
                    console.log(`[${name}] ${Object.keys(pool).length} 个国家`); const candidates={}; for (const [cc,
                    blocks] of Object.entries(pool)) { const ips=sampleFromBlocks(blocks, 8); if (ips.length> 0)
                    candidates[cc] = ips;
                    }
                    ...
                    for (let i = 0; i < batches.length; i++) { const batch=batches[i]; console.log(`[Verify${label}] 批次
                        ${i + 1}/${batches.length} (${batch.length}个IP)...`); const result=await
                        verifyWithMaxMind(batch); for (const [ip, actualCC] of Object.entries(result)) { const
                        expectedCC=ipToCC[ip]; if (actualCC===expectedCC) { if (!verified[actualCC])
                        verified[actualCC]=[]; if (verified[actualCC].length < 10) verified[actualCC].push(ip); } else {
                        console.log(`[Verify${label}] ❌ ${ip}: 期望${expectedCC} 实际${actualCC}`); } } } return verified; }
                        //=============================================// 构建最终数据
                        //=============================================function seededShuffle(arr, seed) { const
                        a=[...arr]; let s=seed>>> 0;
                        ...
                        console.log("\n--- Step 1: 抓取各 RIR 候选IP ---");
                        const rirFetchers = [
                        fetchFromRIR("https://ftp.ripe.net/pub/stats/ripencc/delegated-ripencc-latest", RIPE_COUNTRIES,
                        "RIPE"),
                        fetchFromRIR("https://ftp.apnic.net/stats/apnic/delegated-apnic-latest", APNIC_COUNTRIES,
                        "APNIC"),
                        ...
                        if (allCandidates[cc].length < 15 && !allCandidates[cc].includes(ip)) {
                            allCandidates[cc].push(ip); } } } } const
                            totalCandidates=Object.values(allCandidates).reduce((s, a)=> s + a.length, 0);
                            ...
                            console.log("\n--- 验证抽查 ---");
                            const checkList =
                            ["CN","US","MN","JP","DE","BR","ZA","SC","JM","PR","BB","BS","XK","AQ","IS","GL","KP","TL","PG","NZ","EH"];
