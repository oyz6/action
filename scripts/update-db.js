...
const AFRINIC_COUNTRIES = [
  "ZA","EG","NG","KE","MA","TN","DZ","MU","GH","CI","SN","CM",
  "ZM","ZW","BW","NA","MZ","AO","MW","MG","SC","KM","ST","ET",
  "TZ","UG","RW","BI","SO","DJ","ER","SS","SD","LY","MR","CV",
  "GM","GW","SL","LR","TG","BJ","ML","BF","NE","TD","CF","GQ",
  "GA","CG","CD","SZ","LS",
];
...
// =============================================
// 自动 Bypass：从已缓存的 RIR 原始文本中提取缺失国家的 IP
// =============================================

// 全局缓存，存放每个 RIR 的原始 delegated 文本
let RIR_RAW_TEXT = {};
...
function getAutoBypassIPs(cc) {
  ...
  for (const [rir, text] of Object.entries(RIR_RAW_TEXT)) {
    ...
    for (const line of lines) {
      ...
      const startIP = p[3];
      const count = parseInt(p[4]) || 0;
      if (!isPublicIP(startIP) || count < 16) continue;
...
async function verifyIPs(candidates, label = "") {
  ...
  const verified = {};
  const batches = chunk(allIPs, 100);  // 本地库可一次 100 个，无速率限制
...
async function main() {
  ...
  // Step 1: 并行抓取所有 RIR delegated 文件
  console.log("\n--- Step 1: 抓取各 RIR 候选IP ---");
  const rirFetchers = [
  ...
  if (missing.length > 0) {
    ...
    for (const cc of missing) {
      ...
      if (bypassIPs.length > 0) {
        ...
        console.log(`[BYPASS] ${cc}: ❌ 无法获取任何IP，该地区将缺失`);
      }
    }
  }
  ...
  const payload = {
    ...
    source: "rir-delegated-files verified-by-maxmind-geolite2",
