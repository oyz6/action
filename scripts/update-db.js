...
const HARDCODED_OVERRIDE = {
   ...
   "CF": ["41.188.0.1", "102.178.0.1"],
   "SS": ["102.140.0.1", "105.235.0.1"],
   "CD": ["41.184.0.1", "102.77.0.1"],
   "MG": ["41.207.0.1", "154.126.0.1"],
   "KP": ["175.45.0.1", "210.52.0.1"],
   "ID": ["36.64.0.1", "103.10.0.1"],
...
// Cloudflare 动态抓取相关配置
const CLOUDFLARE_WARP_URL = 'https://www.cloudflare.com/ips-v4';
// ✅ 修改：现在从 Cloudflare 抓取的 IP 将同时用于 EH 和 AQ
const CLOUDFLARE_TARGET_COUNTRIES = ['EH', 'AQ'];
...
// =============================================
// 新增：抓取 Cloudflare WARP IP 库
// =============================================
async function fetchCloudflareWarpIPs() {
  console.log(`[Cloudflare] 抓取 ${CLOUDFLARE_WARP_URL}`);
  const candidates = [];
  try {
    const resp = await fetch(CLOUDFLARE_WARP_URL, {
      signal: AbortSignal.timeout(15000),
      headers: { "User-Agent": "GH-IPCollector/5.0" },
    });
    if (!resp.ok) {
      console.log(`[Cloudflare] HTTP ${resp.status}`);
      return candidates;
    }
    ...
    for (const cidr of lines) {
      ...
      if (!isPublicIP(base) || !bits || parseInt(bits) < 8) continue;
      const size = Math.pow(2, 32 - parseInt(bits));
      const offset = Math.min(Math.floor(size / 2), 500);
      const ip = addOffset(base, offset);
      if (ip) candidates.push(ip);
    }
    console.log(`[Cloudflare] 提取了 ${candidates.length} 个候选IP`);
  } catch (e) {
    console.error(`[Cloudflare] 错误:`, e.message);
  }
...
async function main() {
  ...
  // ==========================================================
  // Step 1.5: 抓取 Cloudflare WARP IP 库
  // ==========================================================
  console.log("\n--- Step 1.5: 抓取 Cloudflare WARP IP 库 ---");
  const cloudflareCandidates = await fetchCloudflareWarpIPs();
  if (cloudflareCandidates.length > 0) {
    for (const target of CLOUDFLARE_TARGET_COUNTRIES) {
      if (!allCandidates[target]) allCandidates[target] = [];
      for (const ip of cloudflareCandidates) {
        if (allCandidates[target].length < 8 && !allCandidates[target].includes(ip)) {
          allCandidates[target].push(ip);
        }
      }
      console.log(`[Cloudflare] 为 ${target} 添加了 ${cloudflareCandidates.length} 个候选IP`);
    }
  }
  ...
  if (missing.length > 0) {
    ...
    for (const cc of missing) {
      ...
      // 【优先级 1】 从 RIR 候选池中动态抓取
      bypassIPs = getAutoBypassIPs(cc);
      if (bypassIPs.length === 0) bypassIPs = getTerritoryFallbackIPs(cc);
      if (bypassIPs.length === 0 && cc !== "XK") {
        bypassIPs = allCandidates[cc] || [];
        if (bypassIPs.length > 0) console.log(`[BYPASS] ${cc} 回退使用候选池IP (${bypassIPs.length} 个)`);
      }
      ...
      // 【优先级 2】 动态抓取失败，使用 Cloudflare WARP 库
      if (!found && CLOUDFLARE_TARGET_COUNTRIES.includes(cc) && cloudflareCandidates.length > 0) {
        console.log(`[BYPASS] ${cc} 动态抓取失败，尝试 Cloudflare WARP 候选...`);
        const confirmedIPs = [];
        for (const ip of cloudflareCandidates) {
          const apiCountry = await queryMRA8(ip);
          // 严格验证逻辑：EH 必须精确匹配，AQ 允许 API 返回 null
          let isValid = false;
          if (cc === 'AQ') {
            isValid = (apiCountry === 'AQ' || !apiCountry);
          } else {
            isValid = (apiCountry === cc);
          }
          
          if (isValid) {
            confirmedIPs.push(ip);
            if (confirmedIPs.length >= 2) break;
          } else {
            console.log(`[BYPASS] 🔍 ${cc} Cloudflare IP ${ip} 实际归属 ${apiCountry || '未知'}，丢弃`);
          }
        }
        if (confirmedIPs.length > 0) {
          verified[cc] = confirmedIPs;
          console.log(`[BYPASS] ✅ ${cc} 验证通过 (Cloudflare WARP): ${confirmedIPs.join(', ')}`);
          found = true;
        }
      }
  ...
  if (finalMissing.length > 0) console.log(`⚠️ 未覆盖: ${finalMissing.join(", ")}`);
  ...
  const payload = {
    ...
    source: "rir-delegated-files + maxmind-geolite2 + mra8-api-verification + cloudflare-warp-dynamic",
