...
async def login(page, email: str, password: str, idx: int) -> Tuple[bool, str, Optional[str]]:
    """登录，返回 (成功, 状态, 截图路径)"""
    # 提取用户名用于登录
...
async def process_account(browser, email: str, password: str, command: str, idx: int) -> dict:
    """处理单个账号"""
    # 提取用户名用于显示和通知
    username = get_username_from_email(email)
    ...
    try:
        # 登录（传入 email，login 内部会提取用户名）
        login_ok, status, login_shot = await login(page, email, password, idx)
...
async def main():
    ...
    for i, (email, _, cmd) in enumerate(accounts, 1):
        username = get_username_from_email(email)
        print(f"  {i}. {mask(username)} | 命令: {mask_command(cmd)}")
